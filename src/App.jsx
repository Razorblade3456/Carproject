...
function App() {
  ...
  const handleAddPart = () => {
    ...
    const expiresAt = parseDateInput(partExpiryDate)
    if (!expiresAt) {
      return
    }
    ...
    const newPart = {
      name: partName.trim(),
      interval: `Expires on ${formatDate(expiresAt)}`,
      isCustom: true,
  ...
  const saveEditedPart = () => {
    ...
    const expiresAt = parseDateInput(editDate)
    if (!expiresAt) return
    const createdAt = Date.now()
    const durationMs = Math.max(expiresAt - createdAt, 0)
                ...
                <div className="parts-card maintenance-card">
                  ...
                  <div className="tracker-summary">
                    ...
                    {partName && partExpiryDate ? (
                      <span>
                        Tracking {partName} · Expires{' '}
                        {formatDate(parseDateInput(partExpiryDate) || Date.now())}
                      </span>
                    ) : (
