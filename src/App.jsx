...
function App() {
  ...
  const options = [
    ...
    },
    {
      ...
      colors: [
        ...
        { name: 'Graphite Steel', hex: '#4b5563' },
        { name: 'Forest Green', hex: '#1f5f4a' },
        { name: 'Copper Dust', hex: '#b9754f' }
      ]
    },
    {
      label: 'SUV',
  ...
  const buildVehicleFromOption = (option) => ({
    id: `${option.label.toLowerCase()}-${Date.now()}`,
    optionLabel: option.label,
    selectedColor: option.colors[0].hex,
    vehicleName: option.model,
    customParts: [],
    partExpirations: {},
    removedParts: []
  })

  const [vehicles, setVehicles] = useState(() => [buildVehicleFromOption(options[0])])
  ...
  const currentOption = options.find((option) => option.label === currentVehicle?.optionLabel) || options[0]
  const availableOptions = options.filter(
    (option) => !vehicles.some((vehicle) => vehicle.optionLabel === option.label)
  )
  ...
  const handleAddVehicle = (option) => {
    const newVehicle = buildVehicleFromOption(option)
    setVehicles((prev) => [...prev, newVehicle])
    setActiveVehicleId(newVehicle.id)
    setShowAddVehicleOptions(false)
    setPartName('')
    setPartExpiryDate('')
    setEditingPart(null)
  }
            ...
            <div className="vehicle-switcher">
              <div className="vehicle-switcher-header">
                ...
                <button
                  ...
                  type="button"
                  onClick={() => setShowAddVehicleOptions((prev) => !prev)}
                  disabled={availableOptions.length === 0}
                >
                  {availableOptions.length === 0 ? 'All vehicles added' : 'Add another vehicle'}
                </button>
              </div>
              ...
              {showAddVehicleOptions ? (
                <div className="options-grid add-vehicle-grid">
                  {availableOptions.length > 0 ? (
                    availableOptions.map((option) => (
                      <button
                        ...
                        type="button"
                        onClick={() => handleAddVehicle(option)}
                      >
                    ...
                    <div className="part-chip">
                      <div className="part-name">All vehicle types added.</div>
                    </div>
                  )}
                </div>
              ) : null}
